...
def process_batch(batch, tokenizer, mask_prob, chunk_size=2000, seq_len=1024):
    chunks = batch['chunks']
    ...
    # Apply masking
    input_ids = tokenized_pairs['input_ids'].clone()
    labels = input_ids.clone()

    probability_matrix = torch.full(input_ids.shape, mask_prob)
    special_tokens_mask = tokenized_pairs['special_tokens_mask']

    # Set probability to zero for special tokens
...
def process_batch(batch, tokenizer, mask_prob, chunk_size=2000, seq_len=1024):
    ...
    probability_matrix = torch.full(input_ids.shape, mask_prob)
    special_tokens_mask = tokenized_pairs['special_tokens_mask'].bool()  # Convert to boolean mask
    ...
    while ts < cfg.nb_updates:
        ...
        for _ in pb:
            ...
            for i in range(update_frequency):
                ...
                b = process_batch(b, tokenizer, cfg.mask_prob, seq_len=cfg.seq_len)
